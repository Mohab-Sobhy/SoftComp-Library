    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            model.createNetwork(3,
                    new int[] { 13, 32, 16 },
                    new int[] { 32, 16, 1 });
            model.getNetwork().getLayers()[model.getNetwork().getLayers().length - 1]
            ...
            System.out.println("Accuracy (total correct): " + ((TN + TP) / predResults.length) * 100 + "%");
            System.out.println("Precision (correct when predicting disease): " + ((TP) / (TP + FP)) * 100 + "%");
            System.out.println("Recall (% of actual disease cases caught): " + ((TP) / (TP + FN)) * 100 + "%");
        } catch (Exception e) {
